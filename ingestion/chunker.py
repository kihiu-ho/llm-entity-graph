...
class SemanticChunker:
    ...
    async def chunk_document(
        ...
        logger.info(f"Chunking config: size={self.config.chunk_size}, overlap={self.config.chunk_overlap}, max_size={self.config.max_chunk_size}")

        # First, try semantic chunking if enabled
        if self.config.use_semantic_splitting and len(content) > self.config.chunk_size:
            try:
                logger.info(f"Using semantic chunking for document with {len(content)} characters")
                ...
                if semantic_chunks:
                    ...
                    logger.info(f"Semantic chunking created {len(chunk_objects)} chunks")
                    return chunk_objects
            except Exception as e:
                logger.warning(f"Semantic chunking failed, falling back to simple chunking: {e}")

        # Fallback to rule-based chunking
        logger.info(f"Using simple chunking for document with {len(content)} characters")
        simple_chunks = self._simple_chunk(content, base_metadata)
        logger.info(f"Simple chunking created {len(simple_chunks)} chunks")
    ...
    async def _semantic_chunk(self, content: str) -> List[str]:
        ...
        logger.info(f"Structural splitting created {len(sections)} sections")
    ...
    def _split_on_structure(self, content: str) -> List[str]:
        """
        Split content on structural boundaries (less aggressive for larger chunks).
        ...
        Returns:
            List of sections
        """
        # Only split on major structural elements to create larger chunks
        patterns = [
            r'\n#{1,3}\s+.+?\n',  # Only major headers (H1-H3)
            r'\n\n\n+',          # Only triple+ newlines (major breaks)
            r'\n```.*?```\n',    # Code blocks (keep these as they're important)
        ]
        ...
        # Filter out very small sections to encourage larger chunks
        min_section_size = self.config.chunk_size // 4  # At least 1/4 of target chunk size
        filtered_sections = []
        accumulated = ""

        for section in sections:
            if len(section.strip()) < min_section_size:
                accumulated += section
            else:
                if accumulated:
                    filtered_sections.append(accumulated)
                    accumulated = ""
        ...
        logger.info(f"Filtered {len(sections)} sections down to {len(filtered_sections)} larger sections")
